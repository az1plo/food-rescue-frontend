...
export class OfferFilterModalComponent {
  ...
  protected readonly distanceChipOptions = computed(() => {
    const options = new Set<number>(this.distanceOptions);
    options.add(this.draftFilters().distanceKm);
    return Array.from(options).sort((first, second) => first - second);
  });
